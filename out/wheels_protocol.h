...
KalmanProtocol_StatusTypeDef WheelsStatus_unpack(const RawDataFrame_TypeDef* raw_data, WheelsStatus_FrameTypeDef* frame) {
    ...
    frame->controller_id = raw_data->data[0 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES];
    frame->driver_status = raw_data->data[1 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES];
    frame->temperature = *((float*)&raw_data->data[2 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES]);
    frame->algorithm_type = raw_data->data[6 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES];
    frame->algorithm_type2 = raw_data->data[7 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES];
    frame->testbool = raw_data->data[8 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES];

    return KALMAN_PROTOCOL_STATUS_OK;
}

KalmanProtocol_StatusTypeDef WheelsStatus_pack(const WheelsStatus_FrameTypeDef* frame, RawDataFrame_TypeDef* raw_data) {
    if (raw_data->capacity < sizeof(WheelsStatus_FrameTypeDef) + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES) {
        return KALMAN_PROTOCOL_STATUS_INVALID_DATA_SIZE;
    }

    raw_data->data[0] = KALMAN_STATUS_REPORT_PROTOCOL_SUBSYSTEM_ID;
    raw_data->data[1] = KALMAN_STATUS_REPORT_WHEELSSTATUS_ID;
    
    raw_data->data[0 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES] = frame->controller_id;
    raw_data->data[1 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES] = frame->driver_status;
    *((float*)&raw_data->data[2 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES]) = frame->temperature;
    raw_data->data[6 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES] = frame->algorithm_type;
    raw_data->data[7 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES] = frame->algorithm_type2;
    raw_data->data[8 + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES] = frame->testbool;

    raw_data->length = sizeof(WheelsStatus_FrameTypeDef) + KALMAN_PROTOCOL_STATUS_REPORT_ID_BYTES;
...
/// Health Checks
////////////////////////////////////////
StatusReportResult_TypeDef WheelsStatus_DriverStatus_HealthCheckResult(const WheelsStatus_FrameTypeDef* frame) {
...
StatusReportResult_TypeDef WheelsStatus_Temperature_HealthCheckResult(const WheelsStatus_FrameTypeDef* frame) {
...
const char* WheelsStatus_Temperature_HealthCheckDescription(const WheelsStatus_FrameTypeDef* frame) {
    if (frame->temperature == KALMAN_STATUS_WHEELSSTATUS_TEMPERATURE_HEALTH_CHECK_OK_1) {
        return KALMAN_STATUS_WHEELSSTATUS_TEMPERATURE_HEALTH_CHECK_OK_1_DESCRIPTION;
    }
    if (frame->temperature == KALMAN_STATUS_WHEELSSTATUS_TEMPERATURE_HEALTH_CHECK_WARNING_2) {
        return KALMAN_STATUS_WHEELSSTATUS_TEMPERATURE_HEALTH_CHECK_WARNING_2_DESCRIPTION;
    }
    if (frame->temperature == KALMAN_STATUS_WHEELSSTATUS_TEMPERATURE_HEALTH_CHECK_CRITICAL_3) {
